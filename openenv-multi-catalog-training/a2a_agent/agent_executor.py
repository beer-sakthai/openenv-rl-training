...
class _Episode:
    ...
    def __init__(self, env_name: str, client):
        ...
        self.extra: dict = {}  # per-env scratch (e.g. echo's secret phrase)
...
class OpenEnvAgentExecutor(AgentExecutor):
    ...
    async def _start_episode(
        ...
        env_name = (
            (context.message.metadata or {}).get("env")
            if context.message
            else None
        )
        if env_name not in ENV_URLS:
            await updater.failed(
                message=new_agent_text_message(
                    f"First message must carry metadata.env in "
                    f"{list(ENV_URLS)}, "
                    f"got {env_name!r}"
                )
            )
    ...
    async def _step_episode(
        ...
        if episode is None:
            await updater.failed(
                message=new_agent_text_message(
                    "Unknown task_id; episode not found."
                )
            )
            return
        if episode.done:
    ...
    async def _reset(self, episode: _Episode) -> str:
        ...
        if name == "echo":
            await client.reset()
            episode.extra["secret"] = "the falcon flies at dawn" # type: ignore
        ...
        if name == "coding":
            await client.reset()
            return (
                "Write Python code that prints the result of 17 * 23, "
                "nothing else."
            )
        if name == "chat":
            await client.reset()
    ...
    async def _step(self, episode: _Episode, action) -> str:
        ...
        if name == "chat":
            ...
            episode.last_reward = (
                getattr(result.observation, "reward", 0.0) or 0.0
            )
            episode.done = result.observation.done
        ...
        if name == "atari":
            ...
            result = await client.step(
                AtariAction(
                    action_id=action_id, game_name=ATARI_GAME, obs_type="ram"
                )
            )
            episode.last_reward = result.observation.reward or 0.0
            episode.done = result.observation.done
        ...
        if name == "repl":
            if (
                ...
                and action.get("final_answer") is not None
            ):
                result = await client.step(
                    REPLAction(
                        code="",
    ...
    def _format_ram(obs) -> str:
        ...
        return (
            f"RAM ({len(ram)} bytes): {' '.join(map(str, ram))}. "
